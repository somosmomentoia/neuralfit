...
export default function ExerciseDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [exerciseRes, meRes] = await Promise.all([
          apiFetch(`/professional/exercises/${params.id}`),
          apiFetch('/auth/me'),
        ]);
  ...
  const canEdit = exercise && (exercise.createdById === currentUserId);

  const handleDelete = async () => {
    if (!exercise) return;
    if (!confirm('¿Estás seguro de eliminar este ejercicio? Esta acción no se puede deshacer.')) return;
    try {
      await apiFetch(`/professional/exercises/${exercise.id}`, { method: 'DELETE' });
      router.push('/professional/exercises');
    } catch (err) {
